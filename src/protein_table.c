                            ...
                            const size_t max_dna_pattern_length)
{
    ...
    if (combinations > max_dna_patterns){
        fprintf(stderr, "ERROR: AA pattern \"%1.*s\" translated to %d DNA patterns, max allowed combinations are %lu!\n",
                (int)aa_pattern_size, aa_pattern, combinations, max_dna_patterns);
        return 0;
    }
